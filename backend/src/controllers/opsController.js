...
async function recentActivity(req, res) {
  ...
  const [users, presenceRows, submissions, candidates] = await (store.pool ? Promise.all([
    safeOpsQuery(`select * from public.users limit 600`),
    safeOpsQuery(`select * from public.presence limit 600`),
...
async function performanceCentre(req, res) {
  ...
  const [users, candidates, tasks, notifications, activityLog, submissions, interviews, presence] = await (store.pool ? Promise.all([
    safeOpsQuery(`select * from public.users limit 600`),
    safeOpsQuery(`select * from public.candidates limit 2500`),
    safeOpsQuery(`select * from public.tasks limit 2000`),
    safeOpsQuery(`select * from public.notifications limit 2500`),
    safeOpsQuery(`select * from public.activity_log limit 3000`),
    safeOpsQuery(`select * from public.submissions limit 3000`),
    safeOpsQuery(`select * from public.interviews limit 2500`),
    safeOpsQuery(`select * from public.presence limit 600`),
  ]) : Promise.all([
    table('users'),
    table('candidates'),
