...
function taskVisibleToUser(task, user, candidate = null) {
  ...
  const directAssigned = String(task?.assigned_to_user_id || '') === String(user?.user_id || '')
    ...
    || String(task?.assigned_to_code || '').trim().toLowerCase() === recruiterCode;
  if (directAssigned) return true;
  return Boolean(candidate && candidateBelongsToUser(candidate, user));
}

async function list(req, res) {
  const items = await table('tasks');
  const candidates = await table('candidates').catch(() => []);
...
async function nextReminder(req, res) {
  ...
  const next = items
    ...
    .map((row) => ({ ...row, dueAt: new Date(row.due_date || 0).getTime(), due_in_minutes: dueInMinutes(row.due_date, now), reminder_buffer_minutes: 10 }))
