...
async function join(req, res) {
  ...
  const freshPresence = await store.findById('presence', 'user_id', req.user.user_id);
  return res.json(compact ? await attendanceGateSnapshot(req, freshPresence || item) : await attendanceSnapshot(req));
}
