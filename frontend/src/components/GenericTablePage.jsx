...
export default function GenericTablePage({ title, subtitle, endpoint, columns, actions, createConfig, rowHref, pollMs = 60000 }) {
  const cacheKey = `genericTable:${endpoint}`;
  const [rows, setRows] = useState(() => readPageCache(cacheKey, []));
  ...
  async function load() {
    setLoading(true);
    try {
      const data = await api.get(endpoint, { allowStale: true, timeoutMs: 22000 });
      const items = data.items || [];
      setRows(items);
      writePageCache(cacheKey, items.slice(0, 1500));
