...
export default function SemiHourlyReportPage() {
  ...
  return (
    <Layout
      ...
      subtitle={reportId ? 'Saved 30 minutes report reopened from Reports.' : 'Manual 30 minutes report. Nothing loads until Generate is clicked.'}
    >
      <style>{`
      ...
      <div className="shr-shell">
        {!reportId ? <div className="row-actions"><button type="button" className="add-profile-btn bounceable" onClick={generateReport}>Generate 30 Minutes Report</button></div> : null}
        ...
        <div className="shr-note">
          {data.generated_at ? (data.is_saved_snapshot ? 'Saved snapshot' : 'Generated') : 'Not generated'} at {data.generated_at ? new Date(data.generated_at).toLocaleString() : '-'} • Period key {data.period_key || '-'}{data.saved_title ? ` • ${data.saved_title}` : ''}
        </div>
