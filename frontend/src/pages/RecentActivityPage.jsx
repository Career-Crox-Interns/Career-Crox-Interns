...
export default function RecentActivityPage() {
  ...
  const [status, setStatus] = useState('Manual-only: no data loaded yet. Click Update Details when needed.');
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      params.set('limit', liveMode ? '200' : '300');
      if (liveMode) params.set('live', '1');
      const data = await api.get(`/api/recent-activity?${params.toString()}`, { cacheTtlMs: 60000, timeoutMs: 45000, retries: 1, allowStale: true, background: false });
      setRows(data.items || []);
      setSummary(data.summary || {});
      setRecruiterOptions(data.recruiters || []);
      setTruncated(Boolean(data.truncated));
      setStatus(`Updated at ${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}. Rows: ${Number(data.items?.length || 0)}. No polling started.`);
    } catch (err) {
      setError(err.message || 'Recent activity load failed.');
      setStatus('Update failed. Check the error below.');
    } finally {
      setLoading(false);
    }
  }, [range.from, range.to, selectedRecruiters, idleThreshold, search, liveMode]);

  // Manual-only: this page does not auto-load or poll.
  // Click Update Details to spend network load intentionally.
  ...
  async function downloadReport() {
    ...
    try {
      await openManagerProtectedExport(`/api/recent-activity/export?${params.toString()}`, 'recent-activity/export');
    } catch (error) {
  ...
  return (
    <Layout title="Recent Activity" subtitle="Manual activity tracking. Data loads only after Update Details.">
      ...
      <div className="table-panel top-gap-small glassy-card fade-up">
        <div className="table-toolbar no-wrap-toolbar activity-toolbar-stack">
          <div>
            ...
            <div className="helper-text">No auto-load, no polling. Change filters, then click Update Details to refresh this section.</div>
            <div className="helper-text sync-message">{status}</div>
          </div>
          <div className="toolbar-actions compact-pills candidate-toolbar-actions activity-head-actions">
            ...
            <button type="button" className="ghost-btn bounceable modern-filter-btn" disabled={loading} onClick={load}>{loading ? 'Updating...' : 'Update Details'}</button>
          </div>
        </div>
        ...
        {!error && !loading && !rows.length ? <div className="helper-text top-gap-small">Activity data is not loaded yet. Click Update Details when you actually need it.</div> : null}
            ...
            <tbody>
              {filteredRows.map((row) => (
                <tr key={row.activity_id}>
                  ...
                  <td>
                    {row.candidate_id ? <a className="mini-btn view bounceable" href={`/candidate/${row.candidate_id}`} target="_blank" rel="noopener noreferrer">Open</a> : <span className="helper-text">-</span>}
                  </td>
                </tr>
              ))}
