...
export default function FollowUpsPage() {
  ...
  useSmartDeltaSync({
    ...
    idKey: 'candidate_id',
    rows: items,
    onRows: (changedRows) => setItems((current) => mergeRowsById(current, changedRows, 'candidate_id')),
  });
          ...
          <table className="crm-table colorful-table dense-table readable-flow-table followups-readable-table">
            ...
            <tbody>
              {filteredRows.map((row) => (
                <tr key={row.candidate_id} className="clickable-row" onClick={() => openCandidateProfile(row)}>
