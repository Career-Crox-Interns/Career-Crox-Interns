...
const SAMPLE_MEDIA = {
  id: 'sample-hr-customer-support-call',
  name: 'sample-hr-customer-support-interview.mp3',
  url: '/assets/media/sample-hr-customer-support-interview.mp3',
  type: 'audio/mpeg',
  size: 0,
  duration: 302,
...
const SAMPLE_ANALYSIS = {
  overall: 81,
  summary: 'Good interview flow with decent confidence, but the recruiter still rushed parts of salary handling and missed one smoother transition.',
  rows: [
    { percentage: 86, area: 'Script Adherence', checked: 'Greeting, intro, BPO question, job-role explanation, salary check, qualification check, closing', result: 'Most mandatory points covered. One relocation follow-up could be cleaner.' },
    { percentage: 79, area: 'Clarity', checked: 'Pace, sentence clarity, question structure, repetition', result: 'Mostly clear. Two questions were bundled together too fast.' },
    { percentage: 75, area: 'Confidence', checked: 'Pauses, filler control, steady delivery', result: 'Recruiter had a few nervous pauses before objection handling.' },
    { percentage: 72, area: 'Objection Handling', checked: 'Salary and shift hesitation handling', result: 'Handled, but with more reassurance needed.' },
    { percentage: 84, area: 'Tone / Professionalism', checked: 'Politeness, empathy, closing warmth', result: 'Professional and polite overall.' },
    { percentage: 88, area: 'Candidate Capture', checked: 'Experience, qualification, salary, joining timeline', result: 'Candidate details captured well.' },
    { percentage: 83, area: 'Closing Quality', checked: 'Next step, callback clarity, ending', result: 'Good close with proper next-step mention.' },
  ],
  notes: [
    'Recruiter asked the expected customer support basics and kept the candidate engaged.',
    'Two transitions felt rushed around salary and shift flexibility.',
    'Candidate communication looked average to good, not very polished but workable for support screening.',
  ],
  suggestions: [
    'Break long questions into single checkpoints.',
    'Use one calmer bridge sentence before salary or shift objections.',
    'Add one final confirmation line so the candidate clearly knows the next step.',
  ],
};
...
export default function QualityAnalystPage() {
  ...
  function loadSample() {
    setUploads([SAMPLE_MEDIA]);
    setSelectedId(SAMPLE_MEDIA.id);
    setAnalysis(SAMPLE_ANALYSIS);
    setMessage('Sample interview loaded with ready analysis.');
  }
  ...
  function analyzeSelected() {
    ...
    const result = selectedMedia.sample ? SAMPLE_ANALYSIS : buildHeuristicAnalysis(selectedMedia, form);
    setAnalysis(result);
    ...
    <Layout title="Quality Analyst" subtitle="Upload calls, preview them, then get a structured QA table with percentages, notes and improvement suggestions.">
      ...
      <div className="panel top-gap">
        ...
        <div className="qa-upload-box top-gap-small">
          ...
          <div className="helper-text top-gap-small">Accepted formats: MP3, WAV, M4A, AAC, OGG, WEBM, MP4, MOV, AVI, MKV, 3GP, MPEG, and MPG. After uploading, use Save and Analyse to process the file.</div>
            ...
            <div className="qa-media-list">
              {uploads.map((item) => (
                <button type="button" key={item.id} className={`qa-media-card ${selectedMedia?.id === item.id ? 'active' : ''}`} onClick={() => { setSelectedId(item.id); setAnalysis(null); }}>
                  ...
                  <div className="qa-media-meta">{item.source === 'built-in sample' ? 'Built-in demo file' : 'Uploaded file'}{item.duration ? ` • ${Math.round(item.duration)}s` : ''}</div>
                </button>
              ))}
            </div>
            {selectedMedia ? (
              <div className="qa-preview">
                <div>
                  ...
                  <div className="qa-media-meta">{selectedMedia.source === 'built-in sample' ? 'Built-in sample for QA preview and rating demo' : 'Current selected media for review'}</div>
                </div>
