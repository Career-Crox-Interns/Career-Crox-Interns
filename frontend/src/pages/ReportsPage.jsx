...
const DEFAULT_CATEGORIES = [
  { key: 'submissions', label: 'Submissions', table_name: 'submissions', record_count: '-' },
  { key: 'interviews', label: 'Interviews', table_name: 'interviews', record_count: '-' },
  { key: 'calls', label: 'Calls', table_name: 'activity_log', record_count: '-' },
  { key: 'selections', label: 'Selections', table_name: 'revenue_hub_entries', record_count: '-' },
  { key: 'joining', label: 'Joining', table_name: 'revenue_hub_entries', record_count: '-' },
  { key: 'allocated_profiles', label: 'Allocated Profiles', table_name: 'candidates', record_count: '-' },
  { key: 'due_profiles', label: 'Due Profiles', table_name: 'candidates', record_count: '-' },
  { key: 'not_interested', label: 'Not Interested', table_name: 'candidates', record_count: '-' },
  { key: 'not_responding', label: 'Not Responding', table_name: 'candidates', record_count: '-' },
  { key: 'attendance_summary', label: 'Attendance Summary', table_name: 'presence', record_count: '-' },
  { key: 'login_timing', label: 'Login Timing', table_name: 'presence', record_count: '-' },
  { key: 'breaks', label: 'Breaks', table_name: 'presence', record_count: '-' },
  { key: 'logout_activity', label: 'Logout Activity', table_name: 'activity_log', record_count: '-' },
];
...
export default function ReportsPage() {
  ...
  const [meta, setMeta] = useState({ recruiters: [], categories: DEFAULT_CATEGORIES, tables_ready: DEFAULT_CATEGORIES, cards: {} });
  const [filters, setFilters] = useState({ recruiter_code: 'all', categories: DEFAULT_CATEGORIES.map((item) => item.key), from: '', to: '', preset: 'all' });
  const [latest, setLatest] = useState('');
  const [busy, setBusy] = useState(false);
  const [status, setStatus] = useState('Manual-only: choose duration/recruiter and click Generate Report or Generate Hold Report. This page will not auto-spend Supabase network load.');
  ...
  async function load({ full = false } = {}) {
    ...
    setError('');
    try {
      const query = full ? 'full=1' : 'light=1';
      const data = await api.get(`/api/reports?${query}&_ts=${Date.now()}`, { cacheTtlMs: 0, timeoutMs: 60000 });
      ...
      setStatus(full ? 'Live report filter options loaded.' : 'Recent reports refreshed.');
    } catch (err) {
      setError(err.message || 'Reports refresh failed.');
      setStatus('Refresh failed. Check the error below.');
    } finally {
      setBusy(false);
    }
  }

  // Manual-only page. Page load does not hit Supabase.

  const selectedAll = filters.categories.length === (meta.categories?.length ? meta.categories : DEFAULT_CATEGORIES).length;

  const selectedLabels = useMemo(() => {
    const selected = new Set(filters.categories);
    return (meta.categories?.length ? meta.categories : DEFAULT_CATEGORIES).filter((item) => selected.has(item.key)).map((item) => item.label);
  }, [filters.categories, meta.categories]);

  async function generateReport(customCategories) {
    setBusy(true);
    setError('');
    setStatus('Generating Excel report. This is a one-time manual Supabase hit.');
    try {
      const payload = {
        ...filters,
        categories: customCategories || filters.categories,
      };
      const data = await api.post('/api/reports/generate', payload, { timeoutMs: 120000, cacheTtlMs: 0 });
      setLatest(data.download_url || 'Report generated.');
      if (data.download_url) triggerDownload(data.download_url);
      setStatus('Report generated successfully. Download/open triggered.');
      await load({ full: false });
    } catch (err) {
      setError(err.message || 'Report generate failed.');
      setStatus('Report generate failed. Check the error below.');
    } finally {
      setBusy(false);
    }
  }
  ...
  async function generateHoldReport() {
    setBusy(true);
    setError('');
    setStatus('Generating Hold Report manually. Supabase will be touched only for this click.');
    try {
      const payload = {
        recruiter_code: filters.recruiter_code,
        from: filters.from,
        to: filters.to,
        preset: filters.preset,
      };
      const data = await api.post('/api/reports/generate-hold', payload, { timeoutMs: 120000, cacheTtlMs: 0 });
      setLatest(data.download_url || 'Hold Report generated.');
      if (data.download_url) triggerDownload(data.download_url);
      setStatus('Hold Report generated successfully. Download/open triggered.');
      await load({ full: false });
    } catch (err) {
      setError(err.message || 'Hold Report generate failed.');
      setStatus('Hold Report failed. Check the error below.');
    } finally {
      setBusy(false);
    }
  }
  ...
  async function generateSemiHourlyReport() {
    setBusy(true);
    setError('');
    setStatus('Generating 30 minutes report manually. No auto-report is running.');
    try {
      const data = await api.get(`/api/reports/semi-hourly?generate=1&_ts=${Date.now()}`, { cacheTtlMs: 0, timeoutMs: 120000 });
      const openUrl = data?.saved_report_id ? `/semi-hourly-report?reportId=${encodeURIComponent(data.saved_report_id)}` : '/semi-hourly-report';
      setLatest(openUrl);
      setStatus('30 minutes report generated successfully. Opening report page.');
      window.open(openUrl, '_blank', 'noopener,noreferrer');
      await load({ full: false });
    } catch (err) {
      setError(err.message || '30 minutes report generate failed.');
      setStatus('30 minutes report failed. Check the error below.');
    } finally {
      setBusy(false);
    }
  }
  ...
  function selectAllCategories() {
    setFilters((prev) => ({ ...prev, categories: (meta.categories?.length ? meta.categories : DEFAULT_CATEGORIES).map((item) => item.key) }));
  }
  ...
  const cards = [
    { className: 'orange', label: 'Last Report Made', value: meta.cards?.last_report_made ? formatDateTime(meta.cards.last_report_made) : 'Manual' },
  ...
  return (
    <Layout title="Reports" subtitle="Download local Excel reports without eating Supabase storage.">
      <style>{`
        ...
        .reports-hero-row{display:grid;grid-template-columns:repeat(4,minmax(0,1fr)) auto auto auto;gap:14px;align-items:stretch;}
        ...
        .reports-hold-top{min-width:210px;border:none;border-radius:20px;padding:16px 18px;font-weight:950;color:#fff;background:linear-gradient(135deg,#ff8a1f,#ffb347,#277cff);box-shadow:0 18px 36px rgba(255,138,31,.25);cursor:pointer;position:relative;overflow:hidden;}
        .reports-hold-top::after{content:"";position:absolute;inset:-40%;background:linear-gradient(90deg,transparent,rgba(255,255,255,.38),transparent);transform:translateX(-65%) rotate(14deg);animation:reportsHoldSweep 2.4s linear infinite;}
        .reports-hold-top span{position:relative;z-index:1;}
        .reports-hold-top:disabled{opacity:.65;cursor:wait;}
        @keyframes reportsHoldSweep{to{transform:translateX(65%) rotate(14deg);}}
        ...
        .reports-error{padding:12px 14px;border-radius:16px;background:rgba(255,72,102,.1);border:1px solid rgba(255,72,102,.25);font-weight:800;color:#b4233c;}
      ...
      <div className="reports-shell">
        <div className="reports-hero-row">
          ...
          {isManager && <button type="button" className="reports-download-top" disabled={busy} onClick={() => generateReport()}>
            {busy ? 'Generating...' : 'Generate Report'}
          </button>}
          {isManager && <button type="button" className="reports-hold-top" disabled={busy} onClick={generateHoldReport}>
            <span>{busy ? 'Generating...' : 'Generate Hold Report'}</span>
          </button>}
          {isManager && <button type="button" className="reports-download-top" disabled={busy} onClick={generateSemiHourlyReport}>
            {busy ? 'Generating...' : 'Generate 30 Minutes Report'}
          </button>}
        </div>

        <div className="reports-filter-panel">
          <div className="reports-latest">{status}</div>
          {error ? <div className="reports-error">{error}</div> : null}
          <div className="reports-category-wrap">
            <button type="button" className="reports-preset-btn" disabled={busy} onClick={() => load({ full: false })}>Refresh Recent Reports</button>
            <button type="button" className="reports-preset-btn" disabled={busy} onClick={() => load({ full: true })}>Load Live Filter Options</button>
          </div>
          ...
          <div className="reports-category-wrap">
            {(meta.categories?.length ? meta.categories : DEFAULT_CATEGORIES).map((item) => (
              <button
                key={item.key}
                type="button"
          ...
          {isManager && latest && <div className="reports-latest">Latest output ready: {String(latest).startsWith('/') ? <a href={latest} target="_blank" rel="noreferrer">Open report</a> : latest}</div>}
        </div>
        ...
        <div className="reports-panels">
          <div className="reports-panel">
            ...
            <table className="reports-table colorful-table">
              ...
              <tbody>
                {(meta.tables_ready?.length ? meta.tables_ready : DEFAULT_CATEGORIES).map((row) => (
                  <tr key={row.key}>
                    <td>{row.label}</td>
          ...
          <div className="reports-panel">
            <div className="reports-panel-head">
              <div>
                ...
                <div className="reports-mini-note">Excel exports plus manually generated 30 minutes reports.</div>
              </div>
            </div>
            <table className="reports-table colorful-table">
              <thead>
