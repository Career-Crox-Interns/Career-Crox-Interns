...
export default function DataExtractorPage() {
  ...
  return (
    <Layout title="Data Extractor" subtitle="Upload resumes, raw documents, public URLs and clean imported data before it hits your CRM.">
      <div className="page-data-extractor">
      <style>{`
...
.dx-segment button{border:none}
        .dx-mode-pill{background:linear-gradient(135deg,#4b86ff 0%, #6f69ff 100%);color:#fff;border:none;box-shadow:0 10px 20px rgba(83,109,255,.24)}
        .dx-mode-pill.dx-mode-pill-raw{background:linear-gradient(135deg,#ffb257 0%, #ff8e55 100%)}
        .dx-mode-pill.dx-mode-pill-url{background:linear-gradient(135deg,#40c891 0%, #34a8d9 100%)}
        .dx-mode-pill.active{background:linear-gradient(135deg,#245bff 0%, #3cc0ff 100%);color:#fff}
      ...
      <div className="panel top-gap">
        ...
        <div className="dx-segment top-gap-small">
          <button type="button" className={`top-pill bounceable dx-mode-pill ${mode === 'files' ? 'active' : ''}`} onClick={() => setMode('files')}>Upload Files</button>
          <button type="button" className={`top-pill bounceable dx-mode-pill dx-mode-pill-raw ${mode === 'raw' ? 'active' : ''}`} onClick={() => setMode('raw')}>Paste Raw Text</button>
          {target === 'client' ? <button type="button" className={`top-pill bounceable dx-mode-pill dx-mode-pill-url ${mode === 'url' ? 'active' : ''}`} onClick={() => setMode('url')}>Public URL</button> : null}
        </div>
        ...
        {target === 'candidate' ? (
          <div className="field top-gap-small dx-default-process-field" style={{ maxWidth: 280 }}>
