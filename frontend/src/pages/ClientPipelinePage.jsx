...
export default function ClientPipelinePage() {
  ...
  useSmartDeltaSync({
    scope: 'client-pipeline',
    idKey: 'lead_id',
    rows,
    onRows: (changedRows) => setRows((current) => mergeRowsById(current, changedRows, 'lead_id')),
  });
        ...
        <div className="cp-grid">
          <div className="cp-panel">
            ...
            <div className="cp-quick-bar">
              ...
              {isManager ? <button type="button" className="cp-btn ghost" onClick={async () => { try { await openManagerProtectedExport('/api/client-pipeline/export', 'client-pipeline/export'); } catch (error) { setNotice(error.message || 'Export failed.'); } }}>Export Excel/CSV</button> : null}
          ...
          <div className="cp-panel">
            ...
            {isManager ? (
              <>
                ...
                <div className="cp-quick-bar">
                  <button type="button" className="cp-btn primary" disabled title="Data Extractor disabled to save Supabase network load">Data Extractor Disabled</button>
                  <button type="button" className="cp-btn ghost" disabled title="Review import is disabled to save Supabase network load">Review Disabled</button>
                </div>
              </>
            ) : (
